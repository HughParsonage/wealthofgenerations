...
#' @param tidy (logical)
#' @param span the spanning function passed to supsmu


expand.MEANS.SUPSMU <- function(age.totals,age.labels,k=30,year="",weekly=TRUE,wts=NULL,tidy=TRUE,span="cv",...){
  ...
  # span is the smoothing factor in supsmu
  ...
  # 4) SMOOTH
  single.MODELLED <- supsmu(x=ageall,y=single.EXPANDED,span=span)$y
